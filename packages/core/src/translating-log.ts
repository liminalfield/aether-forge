 ...
 *
 * Failures carry structured fields rather than a message, because a caller
 * wanting to react to one should not be reading English. Something eventually
 * has to show a person what happened, and this is that.
 *
 * The final branch is a compile-time check: adding a failure kind without
 * describing it here will not build.
 */
export function describeFailure(failure: TranslatingLogFailure): string {
  switch (failure.kind) {
    case 'storage-failed':
