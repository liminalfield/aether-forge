...
MIT packages must never depend on GPL code — license direction follows dependency direction, which
is mechanically enforced (`pnpm depcheck`). Each package carries its own `LICENSE` and `license`
field. The root `LICENSE` is the GPL-3.0 text that governs the distributed application.
