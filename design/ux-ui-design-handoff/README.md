...
**Do not port this file.** The task is to recreate these designs in the target environment, with
these screens as the visual target. The codebase and the stack now exist, so the note under
"Recommended shape" below is history rather than instruction.

Three documents at the repository root define the product: `00-PROJECT-BRIEF.md` (vision,
constraints, the "assisted but never automatic" principle), `02-MODULE-CONTRACT.md` (the
architecture the app is built to), and `03-DESIGN-BRIEF.md` (the design brief this work answers).
Read the project brief first. Where this README and the briefs disagree, the briefs are the product
truth and this README is the visual truth.
...
> **Settled since this was written.** The stack is chosen and the repository exists: Electron with
> React and TypeScript, SQLite in the main process, pnpm workspace. The renderer is a plain web app
> that does not know Electron exists. See `00-PROJECT-BRIEF.md`. The paragraph below is kept as the
...
The three source briefs that came with this bundle were copies, and they are gone. Read the live
ones at the repository root instead: `00-PROJECT-BRIEF.md`, `02-MODULE-CONTRACT.md` and
`03-DESIGN-BRIEF.md`. The copies had already drifted from them within a day, which is the whole
reason each fact gets one home.
...
## Attribution for the game content in the mockups

The screens reproduce text from Ironsworn: Starforged to make them concrete. A complete oracle table
(`starforged/oracles/derelicts/inhabitants`, all six rows and their ranges), a sector truth and its
result, five asset names with their ability text, several move names, and the composition of the
asset deck.

That text is from the Ironsworn: Starforged SRD by Shawn Tomkin, licensed
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/), reached here through the Datasworn
community dataset. This note is the attribution that licence requires.

Everything else in the mockups is the maintainer's own play: Vasha, the Kingfisher, Oyelaran, the
CHORUS vow, the Vela Ascending, the Grave-of-Ships.

**Icon extraction is a separate licensing question and is not settled.** The icons in this document
are placeholder SVG glyphs, and nothing from the asset PDFs or the worksheets is embedded here. The
note under Fidelity that says to extract the real set during build should not be acted on until
someone checks what licence covers those images, because the SRD covering the text does not
automatically cover the artwork printed alongside it.
